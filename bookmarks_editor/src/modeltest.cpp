...
void ModelTest::parent()
{
    ...
    if (model->rowCount(topIndex) > 0) {
        QModelIndex childIndex = model->index(0, 0, topIndex);
        kDebug() << "(index, parent, model->parent(childIndex))" << childIndex << topIndex <<  model->parent(childIndex);
        kDebug() << "(index, parent, model->parent(childIndex))" << childIndex.data() << topIndex.data() <<  model->parent(childIndex).data();
...
void ModelTest::checkChildren(const QModelIndex &parent, int currentDepth)
{
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int c = 0; c < columns; ++c) {
            ...
            kDebug() << index << index.data() << parent << parent.data() << model->parent(index);
