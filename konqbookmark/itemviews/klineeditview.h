...
#ifndef KLINEEDITVIEW_H
#define KLINEEDITVIEW_H
 ...
 *
 * To support these new features KLineEditView also emits a few more
 ...
 * the completionView(bool) member function for the first time or
 * use setCompletionView(KCompletionView*, bool) to assign your own
 * completion view.  Additionally, to make this widget more functional,
 * KLineEditView will by default handle the text rotation and completion
 * events internally when a completion view is created through either one
 ...
 *
 * A read-only KLineEditView will have the same background color as a
 * disabled KLineEditView, but its foreground color will be the one used
 ...
 *
 * KLineEditView has also a password mode which depends of globals KDE settings.  Use
 * KLineEditView::setPasswordMode instead of QLineEdit::echoMode property to have a password field.
 *
 * \b Usage \n
 *
 ...
 *
 * \code
 * KLineEditView* edit = new KLineEditView(this);
 * KCompletionView* comp = edit->completionView();
 ...
 * KCompletionModel* compModel = new KCompletionModel(this);
 * compModel->setCompletion(comp);
 * KCompletionView* compView = new KCompletionView(this);
 * compView->setModel(compModel);
 * edit->setCompletionView(comp);
 * // Connect to the return pressed signal - optional
 * connect(edit,SIGNAL(returnPressed(const QString&)),comp,SLOT(addItem(const QString&)));
 * \endcode
 *
 * Note if you specify your own completion object you have to either delete
 * it when you don't need it anymore, or you can tell KLineEditView to delete it
 * for you:
 * \code
...
class KONQBOOKMARK_EXPORT KLineEditView : public QLineEdit, public KCompletionBase //krazy:exclude=qclasses
{
    friend class KComboBox;
    friend class KLineEditViewStyle;
...
public:
     ...
     */
    explicit KLineEditView(const QString &string, QWidget *parent = 0);

    /**
     * Constructs a line edit
     * @param parent The parent widget of the line edit.
     */
    explicit KLineEditView(QWidget *parent = 0);
    ...
    */
    ...
    /**
     * By default, KLineEditView recognizes @p Key_Return and @p Key_Enter and emits
     * the returnPressed() signals, but it also lets the event pass,
     * for example causing a dialog's default-button to be called.
     *
     * Call this method with @p trap = @p true to make @p KLineEditView stop these
    ...
    /**
     * @returns the completion-view, that is used in completion mode
     * KGlobalSettings::CompletionPopup.
     * This method will create a completion-view if none is there, yet.
     *
     * @param create Set this to false if you don't want the box to be created
     *               i.e. to test if it is available.
     */
    KCompletionView* completionView(bool create = true);
...
Q_SIGNALS:
    /**
     * Emitted whenever the completion view is activated.
     */
    void completionViewActivated(const QString &);
    ...
    /**
     * Emitted when the completion key is pressed. Should be used to update
     ...
     * In KLineEditView's case this can be either one of two values:
...
public Q_SLOTS:
     ...
     */
    void updateCompletedItems(bool autoSuggest = true);
...
protected Q_SLOTS:
    ...
    * a given list. Should be called when updating the completion model if it's
    * done asynchronously for example, otherwise it's usually called by 
    * doCompletion().
    */
